...
class CustomizedApplicationMainIT extends BaseIntegrationControllerSpec {
    ...
    def "should save few users and get all of them"() {
        ...
        users.each {
            def userId = postAndReturnLong(CUSTOM_USERS_URL, it)
            it.setId(userId)
        }

        when:
        def foundUsers = getAndReturnArrayJson(CUSTOM_USERS_URL)
